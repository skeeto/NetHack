...
static char text_queue[128];
static int tq_len = 0, tq_pos = 0;
...
static Uint64 _stored_timestamp;
...
_utf8_next(void)
{
    ...
    if (tq_pos >= tq_len)
        return -1;
    byte = (unsigned char) text_queue[tq_pos];
    if (byte >= 0xf0) {
        bytes = 4;
        unicode = byte & 0x7;
    } else if (byte >= 0xe0) {
        ...
        tq_pos = tq_len = 0; /* stray continuation byte; drop it */
        return -1;
    } else {
        bytes = 1;
        unicode = byte;
    }
    if (tq_pos + bytes > tq_len) {
        tq_pos = tq_len = 0;
        return -1;
    }
    for (i = 1; i < bytes; i++)
...
/* return one queued text character, with the 2 ms wait used to merge
   keypad +-/* text with their PADxxx key events */
static int
_process_text_queue(void)
{
    int key = _utf8_next();
    int rval = (key < 0) ? -1 : _handle_alt_keys(key);

    if (rval > 0 && rval < 0x80 && strchr("/+*-", rval)) {
        _stored_key = rval;
        _stored_timestamp = SDL_GetTicks();
        rval = -1;
    }
    ...
    for (i = 0; key_table[i].keycode; i++) {
        if (key_table[i].keycode == event.key.key) {
            if ((event.key.mod & SDL_KMOD_SHIFT)
                ...
                /* text input will deliver these as characters */
...
_process_mouse_event(void)
{
    ...
    if (event.type == SDL_EVENT_MOUSE_WHEEL) {
        wheel_x = (event.wheel.x > 0.0f) - (event.wheel.x < 0.0f);
        wheel_y = (event.wheel.y > 0.0f) - (event.wheel.y < 0.0f);
    ...
    /* mouse coords are in window points; cells are in pixels */
    {
        float ptw = 1.0f, pth = 1.0f;
        int pw, ph, pxw, pxh;

        SDL_GetWindowSize(pdc_window, &pw, &ph);
        SDL_GetWindowSizeInPixels(pdc_window, &pxw, &pxh);
        if (pw > 0 && ph > 0) {
            ptw = (float) pxw / (float) pw;
            pth = (float) pxh / (float) ph;
        }
        SP->mouse_status.x = (int) (fx * ptw) / pdc_fwidth;
        SP->mouse_status.y = (int) (fy * pth) / pdc_fheight;
    }
    ...
    if (event.type == SDL_EVENT_MOUSE_MOTION) {
        ...
        short action = event.button.down ? BUTTON_PRESSED : BUTTON_RELEASED;
        Uint8 btn = event.button.button;

        if (btn < 1 || btn > 3)
            return -1;

        /* check for a click -- a press followed quickly by a release */
        if (action == BUTTON_PRESSED && SP->mouse_wait) {
            SDL_Event rel;

            while (action != BUTTON_TRIPLE_CLICKED
                   && SDL_WaitEventTimeout(&rel, SP->mouse_wait)) {
                if (rel.type == SDL_EVENT_MOUSE_BUTTON_UP
                ...
                } else if (rel.type != SDL_EVENT_MOUSE_BUTTON_DOWN
                           || rel.button.button != btn) {
                    SDL_PushEvent(&rel);
                    break;
                }
...
PDC_get_key(void)
{
    ...
    if (_stored_key && SDL_GetTicks() > _stored_timestamp + 2) {
        const int key = _stored_key; /* +-/* not merged with a PADxxx */
    ...
    case SDL_EVENT_QUIT:
        event.type = 0;
        if (++quit_requests >= 4)
            exit(1); /* several ignored requests: force it */
        if (pdc_sdl3_quit_hook)
            (*pdc_sdl3_quit_hook)();
        if (PDC_get_function_key(FUNCTION_KEY_SHUT_DOWN))
            return PDC_get_function_key(FUNCTION_KEY_SHUT_DOWN);
        /* the hook raised NetHack's deferred hangup; return ESC so the
           blocked input call wakes up, any pending prompt is canceled,
           and the command loop notices the hangup and saves right away */
        return 0x1b;
    ...
    case SDL_EVENT_WINDOW_RESIZED:
    {
        ...
        if ((pdc_sheight / pdc_fheight != prev_rows
    ...
    case SDL_EVENT_TEXT_INPUT:
    {
        ...
        if (n > sizeof text_queue)
            n = sizeof text_queue;
        memcpy(text_queue, event.text.text, n);
        tq_len = (int) n;
        tq_pos = 0;
        event.type = 0;
        PDC_mouse_set();
...
/* discard any pending keyboard or mouse input */
void
PDC_flushinp(void)
{
    while (PDC_check_key())
