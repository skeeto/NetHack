...
#ifdef DLBMEM
/* the archive image generated by bin2c from nhdat */
extern const unsigned char nhdat_blob[];
extern const unsigned long nhdat_blob_len;

/* which member of dlb_libs[] marks it as in use */
#define LIB_INUSE(lp) ((lp)->mdata != 0)
#else
#define LIB_INUSE(lp) ((lp)->fdata != 0)
#endif
...
#ifdef DLBMEM
    /* parse the same header/directory layout out of the embedded
       image; bin2c NUL-terminates the blob so sscanf stays bounded */
    const char *p = (const char *) lp->mdata;
    int used;

    if (sscanf(p, "%ld %ld %ld %ld %ld%n", &lp->rev, &lp->nentries,
               &lp->strsize, &liboffset, &totalsize, &used) != 5)
        return FALSE;
    p += used;
    if (*p == '\n')
        p++;
    if (totalsize != (long) nhdat_blob_len)
        return FALSE; /* truncated or mismatched image */
#else
...
#ifdef DLBMEM
        lp->dir[i].handling = *p++;
        if (sscanf(p, "%s %ld%n", sp, &lp->dir[i].foffset, &used) != 2) {
#else
...
find_file(const char *name, library **lib, long *startp, long *sizep)
{
    ...
    for (i = 0; i < MAX_LIBS && LIB_INUSE(&dlb_libs[i]); i++) {
        lp = &dlb_libs[i];
...
/*
 * Open the library file once using stdio (DLBLIB), or parse the
 * embedded archive image (DLBMEM).  Keep track of the file position.
 */
staticfn boolean
lib_dlb_init(void)
{
...
#ifdef DLBMEM
    dlb_libs[0].mdata = nhdat_blob;
    if (!readlibdir(&dlb_libs[0])) {
        dlb_libs[0].mdata = 0;
        return FALSE;
    }
#else
...
lib_dlb_fread(char *buf, int size, int quan, dlb *dp)
{
    ...
    nread = quan;
    nbytes = nread * size;
    (void) memcpy(buf, dp->lib->mdata + pos, (size_t) nbytes);
    dp->mark += nbytes;
#else
    if (dp->lib->fmark != pos) {
